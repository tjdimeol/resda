...
class SegCriterion(FairseqCriterion):
    def __init__(
        ...
        print("task.cfg.category_list is ", task.cfg.category_list)
        self.id2rawtext = [x.strip() for x in task.cfg.category_list.split(',')]
        print("self.id2rawtext is ", self.id2rawtext)
        print("len(self.id2rawtext) and self.num_seg are ", len(self.id2rawtext), self.num_seg)
    ...
    def upsample_logits(self, logits, hp=32, wp=32, h=512, w=512):
        logits_ = logits[:, :-1]
        #print("\nin upsample_logits, logits_ is ", logits_)
        ...
        #print("\nin upsample_logits, logits_ is ", logits_)
        #print("\nafter comps, what is logits[:, -1:]?", logits[:, -1:])
        logits = torch.cat([logits_, logits[:, -1:]], dim=1)
        #print("\nafter comps, what is logits?", logits)
    ...
    def compute_loss(self, model, net_output, sample, update_num, reduce=True, ema_model=None):
        ...
        if target_lowres is not None:
            ...
            print("target_lowres_shape and classifier_scores_lowres.shape[:-1] are:", target_lowres_shape, classifier_scores_lowres.shape[:-1])
            time.sleep(3)
        ...
        if resnet_postprocess_probability is not None:
            #print("resnet_postprocess_probability  before upsampling is ", resnet_postprocess_probability)
            resnet_postprocess_probability = self.upsample_logits(resnet_postprocess_probability, hp=hp, wp=wp, h=h, w=w) # bilinear upsample
            #print("resnet_postprocess_probability after upsampling is ", resnet_postprocess_probability)
        if not model.training:
        ...
        #print("\ntarget_shape is ", target_shape, "classifier_scores.shape[:-1] is ", classifier_scores.shape[:-1])
        #time.sleep(20)
