...
class SegmentationDataset(OFADataset):
    def __init__(
        ...
        if self.split == 'train':
            #print("split is train, in segmentation_dataset, sleeping 10s")
            #time.sleep(10)
            ...
            #print("split is NOT train, in segmentation_dataset, sleeping 10s")
            #time.sleep(10)
        ...
        print("in segmentation_dataset.py, len(self.id2rawtext) and self.num_seg+1 are ", len(self.id2rawtext), self.num_seg+1)
        ...
        #self.id2seg = np.array([f'<seg_{idx}>' for idx in range(self.num_seg)]) tried just for udd, put back after.. (tjd)
        #print("id2seg is ", self.id2seg)
        self.seg2code = self.encode_text(" ".join(self.id2seg), use_bpe=False)
        #print("self.seg2code is ", self.seg2code)
        #print("len(self.seg2code) is ", len(self.seg2code))
    ...
    def __getitem__(self, index):
        ...
        #print("in segmentation_dataset.py: uniq_id and index are ", uniq_id, index)
        ...
        #print("segmentation just after Image.open... is ", segmentation)
        segmentation_arr = np.asarray(segmentation).copy()
        #print("segmentation_arr just after np.asarray is ", segmentation_arr)
        ...
        if self.split == 'train':
            ...
            #print("length of gt_semantic_seg after pop is ", len(gt_semantic_seg))
            gt_semantic_seg = torch.from_numpy(gt_semantic_seg.astype(np.int64))
            #print("length of gt_semantic_seg is ", len(gt_semantic_seg))
            thing2 = gt_semantic_seg.unsqueeze(0)
            #print("thing2 is ", thing2)
            thing3 = gt_semantic_seg.unsqueeze(0).flatten()
            #print("thing3 is ", thing3)
            #print("length of thing3 is ", len(thing3))
            #gt_semantic_seg_downsampled = thing3
            gt_semantic_seg_downsampled = self.downsample_gt_seg(gt_semantic_seg.unsqueeze(0)).flatten()
            #print("gt_semantic_seg_downsampled is ", gt_semantic_seg_downsampled)
            #print("length of gt_semantic_seg_downsampled is ", len(gt_semantic_seg_downsampled))
            seg_ids_downsampled = self.seg2code[gt_semantic_seg_downsampled]
            #print("seg_ids_downsampled is ", seg_ids_downsampled)
            downsampled_target = torch.cat([seg_ids_downsampled, self.eos_item])
            #print("downsampled_target is ", downsampled_target)
            #print("len(downsampled_target) is ", len(downsampled_target))
            prev_output_item = torch.cat([self.bos_item, seg_ids_downsampled])
            #print("prev_output_item is ", prev_output_item)
            #print("len(prev_output_item) is ", len(prev_output_item))

        else:
            #print("here2")
            #time.sleep(10)
            img_dict = self.image_transform(results)
            #print("img_dict is ", img_dict)
        ...
        #print("in segmentatio_dataset.py, len(self.id2text) is ", len(self.id2text))
        target = torch.cat([seg_ids, self.eos_item])

        # build 
        prompt_ids = [idx for idx in range(len(self.id2text))]
        #print("prompt_ids is ", prompt_ids)
        
        src_text = [self.bos_item]
        if self.prompt is not None:
            #print("src_text is ", src_text)
            src_text += [self.prompt]
            #print("src_text (after) is ", src_text)
            
        #print("len(prompt_ids) is ", len(prompt_ids))
        #print("len(self.id2text) is ", len(self.id2text))
        src_text += [self.id2text[idx] for idx in prompt_ids]
        src_text += [self.eos_item]
        #print("src_text is ", src_text)
        #print("len(src_text) is ", len(src_text))

        src_item = torch.cat(src_text)

        #print("len(src_item) is ", len(src_item))
